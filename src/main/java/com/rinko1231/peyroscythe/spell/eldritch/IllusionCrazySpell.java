...
@AutoSpellConfig
public class IllusionCrazySpell extends AbstractSpell {
    private final ResourceLocation spellId = PeyroScythe.id( "illusion_crazy");
    private final DefaultConfig defaultConfig;

    public IllusionCrazySpell() {
    ...
    public boolean allowLooting() {
        return PeyroScytheConfig.illusionCrazyAllowLooting.get();
    }
    ...
    public void onCast(Level world, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        ...
        if (targetEntityData instanceof TargetEntityCastData targetData) {
            ...
            if (target != null && !target.getType().is(TagsRegistry.ILLUSION_IMMUNE)) {
                ...
                // 设置当前施法者 UUID
                tag.putUUID("peyroscythe:crazy_caster", entity.getUUID());

                // 移除恐惧效果
                if(target.hasEffect(MobEffectRegistry.ILLUSION_FEAR))
                   target.removeEffect(MobEffectRegistry.ILLUSION_FEAR);

                // 添加疯狂效果
                target.addEffect(new MobEffectInstance(MobEffectRegistry.ILLUSION_CRAZY, this.getDuration(spellLevel, entity), this.getAmplifier(spellLevel, entity)));
            }
        }
    ...
    public int getDuration(int spellLevel, LivingEntity caster) {
        return (int) (this.getSpellPower(spellLevel, caster) * 1.0F * 20.0F);
    }
}
