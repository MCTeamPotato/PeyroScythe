...
import com.rinko1231.peyroscythe.spellentity.ice.FrostFogEntity;
...
public class TheFrozenWorldSpell extends AbstractSpell {
    private static final ResourceLocation ID = PeyroScythe.id( "the_frozen_world");
    ...
    public void onCast(Level level, int spellLevel, LivingEntity caster, CastSource castSource, MagicData data) {
        ...
        List<LivingEntity> targets2 = level.getEntitiesOfClass(
                LivingEntity.class,
                caster.getBoundingBox().inflate(r, r, r),
                e -> e != caster
                        && e.distanceToSqr(center) <= rSqr
                        && e.distanceToSqr(center) > innerRSqr
                        && !DamageSources.isFriendlyFireBetween(caster, e)
        );

        for (LivingEntity t2 : targets2)
        {
            t2.addEffect(new MobEffectInstance(
                    CHILLED,
                    PeyroScytheConfig.frozenWorldIceFogDuration.get(),
                    3, false, true, true
            ));
        }
